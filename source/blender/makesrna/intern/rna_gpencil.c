...
static float rna_GPencil_stroke_point_weight_get(bGPDstroke *stroke,
                                                 ReportList *reports,
                                                 int vertexgroup_index,
                                                 int point_index)
{
  ...
  if (dvert->totweight <= vertexgroup_index || vertexgroup_index < 0) {
  ...
  MDeformWeight *dw = BKE_defvert_find_index(pt_dvert, vertexgroup_index);
  if (dw) {
    return dw->weight;
  }
...
static void rna_GPencil_stroke_point_weight_set(
    bGPDstroke *stroke, ReportList *reports, int vertexgroup_index, int point_index, float weight)
{
  ...
  MDeformVert *pt_dvert = stroke->dvert + point_index;
  MDeformWeight *dw = BKE_defvert_ensure_index(pt_dvert, vertexgroup_index);
  if (dw) {
    dw->weight = weight;
  }
}
...
static void rna_def_gpencil_stroke_points_api(BlenderRNA *brna, PropertyRNA *cprop)
{
  ...
  func = RNA_def_function(srna, "weight_get", "rna_GPencil_stroke_point_weight_get");
  RNA_def_function_ui_description(func, "Get vertex group point weight");
  RNA_def_function_flag(func, FUNC_USE_REPORTS);
  RNA_def_int(
      func, "vertexgroup_index", 0, 0, INT_MAX, "VertexGroup", "Vertex Group index", 0, INT_MAX);
  RNA_def_int(func, "point_index", 0, 0, INT_MAX, "Index", "Point index", 0, INT_MAX);
  parm = RNA_def_float(
      func, "weight", 0, -FLT_MAX, FLT_MAX, "Weight", "Point Weight", -FLT_MAX, FLT_MAX);
  RNA_def_function_return(func, parm);

  func = RNA_def_function(srna, "weight_set", "rna_GPencil_stroke_point_weight_set");
