...
static void wm_contact_sheet_pdf_cancel(bContext * /*C*/, wmOperator *op)
{
  if (op->customdata) {
...
{
  ContactSheetParams *load_data = (ContactSheetParams *)MEM_callocN(sizeof(ContactSheetParams),
                                                                    __func__);
...
static int wm_contact_sheet_pdf_invoke(bContext *C, wmOperator *op, const wmEvent * /*event*/)
{
  Scene *scene = CTX_data_scene(C);
  ED_fileselect_ensure_default_filepath(C, op, ".pdf");
...
static void wm_contact_sheet_pdf_draw(bContext * /*C*/, wmOperator *op)
{
